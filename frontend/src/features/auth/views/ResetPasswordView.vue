  ...
  <div class="relative min-h-[calc(100vh-72px)] flex items-center justify-center p-4 font-sans overflow-hidden pt-32 bg-[rgb(9,9,42)] border-b border-[#a6249d]/20">
    <div class="relative w-full max-w-[420px] p-8 space-y-8 bg-[#1A0B2E]/90 backdrop-blur-lg rounded-3xl border border-[#a6249d]/40 shadow-[0_8px_30px_-5px_rgba(34,48,91,0.2)]">
      <div class="text-center">
        <h1 class="font-['League_Spartan'] text-2xl font-bold text-white mb-2">Restablecer contraseña</h1>
        <p class="text-white/50 text-sm">Ingresa el código que recibiste y tu nueva contraseña.</p>
      </div>
      <form @submit.prevent="handleResetPassword" class="space-y-5">
        ...
        <div class="space-y-1.5">
          <label for="code" class="block text-xs font-bold text-white/70 uppercase tracking-wider ml-1">Código de verificación</label>
          <input id="code" v-model="code" type="text" maxlength="4" required class="block w-full pr-4 py-3 bg-[#0a0a1a]/50 border border-white/10 rounded-xl text-white placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-[#a6249d]/50 focus:border-[#a6249d] transition-all duration-200" placeholder="1234" />
        </div>
        <div class="space-y-1.5">
          <label for="newPassword" class="block text-xs font-bold text-white/70 uppercase tracking-wider ml-1">Nueva contraseña</label>
          <input id="newPassword" v-model="newPassword" type="password" required class="block w-full pr-4 py-3 bg-[#0a0a1a]/50 border border-white/10 rounded-xl text-white placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-[#a6249d]/50 focus:border-[#a6249d] transition-all duration-200" placeholder="••••••••" />
        </div>
        <div class="space-y-1.5">
          <label for="confirmPassword" class="block text-xs font-bold text-white/70 uppercase tracking-wider ml-1">Confirmar contraseña</label>
          <input id="confirmPassword" v-model="confirmPassword" type="password" required class="block w-full pr-4 py-3 bg-[#0a0a1a]/50 border border-white/10 rounded-xl text-white placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-[#a6249d]/50 focus:border-[#a6249d] transition-all duration-200" placeholder="••••••••" />
        </div>
...
const handleResetPassword = async () => {
  ...
  try {
    ...
    if (res.data?.ok) {
      ...
      errorMsg.value = res.data?.message || "No se pudo restablecer la contraseña.";
    }
  } catch (error) {
    errorMsg.value = error.response?.data?.message || "No se pudo restablecer la contraseña.";
  } finally {
    loading.value = false;
  }
};
