  ...
  <section class="min-h-screen pt-[96px] bg-[#09092a] font-sans pb-20">
    <div class="mx-auto max-w-[1240px] px-6 py-8">
      ...
      <header class="mb-10 text-center md:text-left">
        <h1 class="font-['League_Spartan'] text-4xl font-bold text-white mb-2">Mis Postulaciones</h1>
        <p class="text-white/50 text-lg">Seguimiento de tus oportunidades laborales</p>
      </header>

      <!-- CONTENT STATES -->
      
      <!-- 1. GUEST STATE (No autenticado) -->
      <div v-if="!isAuthenticated" class="flex flex-col items-center justify-center py-20 text-center bg-[#121225]/50 rounded-3xl border border-white/5 shadow-2xl backdrop-blur-sm">
        <div class="w-20 h-20 bg-gradient-to-br from-[#a0218b] to-[#5a124d] rounded-full flex items-center justify-center mb-6 shadow-lg shadow-[#a0218b]/30">
        ...
        <p class="text-white/60 max-w-md mb-8 leading-relaxed">
          Para ver el estado de tus postulaciones y gestionar tus procesos de selección, necesitas iniciar sesión en tu cuenta.
        </p>
      ...
      <div v-else-if="loading" class="flex flex-col items-center justify-center py-32">
        ...
        <p class="mt-4 text-white/50 animate-pulse">Cargando tus postulaciones...</p>
      </div>

      <!-- 3. ERROR STATE -->
      <div v-else-if="error" class="bg-red-500/10 border border-red-500/20 rounded-xl p-6 text-center text-red-200">
        <p class="font-bold mb-2">Algo salió mal</p>
        <p class="text-sm opacity-80">{{ error }}</p>
        <button @click="cargarPostulaciones" class="mt-4 text-sm underline hover:text-white">Intentar nuevamente</button>
      </div>

      <!-- 4. EMPTY STATE (Autenticado pero sin datos) -->
      <div v-else-if="postulacionesUsuario.length === 0" class="flex flex-col items-center justify-center py-20 text-center bg-[#121225]/50 rounded-3xl border border-white/5 border-dashed">
        ...
        <p class="text-white/50 max-w-sm mb-8">
          Explora las ofertas disponibles y da el primer paso hacia tu próximo empleo.
        </p>
        <router-link 
          to="/ofertas" 
      ...
      <div v-else class="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <main class="flex-1 w-full">
          <!-- Controls Bar -->
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 bg-[#121225] p-4 rounded-xl border border-white/5">
            <h2 class="text-white font-bold text-lg">
              <span class="text-[#b62667]">{{ postulacionesFiltradas.length }}</span> Solicitudes activas
            </h2>
