...
async function updateCandidate(cardKey) {
  try {
    ...
    if (cardKey === 'personal') {
      ...
      const loc = [profileData.distrito, profileData.departamento].map(s => s?.trim()).filter(Boolean).join(', ');
      payload.location = loc || null;
    }
    
    else if (cardKey === 'academic') {
      const anio = profileData.anio_egreso ? parseInt(profileData.anio_egreso, 10) : null;
    ...
    const msg = err.response?.data?.error || err.response?.data?.detail || err.message || 'Error al guardar los cambios';
  ...
  <div class="min-h-screen bg-[rgb(9,9,42)] text-white font-sans pt-32">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
      ...
      <header class="flex flex-col sm:flex-row items-center gap-6 mb-12 justify-between">
        <div class="flex items-center gap-6">
          <img class="h-24 w-24 rounded-full object-cover border-2 border-[#a6249d]/40" src="/src/assets/avatar-placeholder.jpg" alt="Avatar">
          <div class="text-center sm:text-left">
              <h1 class="text-4xl font-bold bg-gradient-to-r from-[#d93340] to-[#a6249d] bg-clip-text text-transparent">
                  {{ fullName }}
              </h1>
      ...
      <main class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="bento-card md:col-span-7">
            <div class="card-header">
             ...
             <div class="card-content">
                <ul class="info-list">
                    <li><span>Carrera</span><strong v-if="!editingCards.academic">{{ profileData.carrera || 'No definida' }}</strong><select v-else v-model="profileData.carrera" class="edit-input"><option value="">Seleccionar carrera</option><option v-for="c in carrerasOptions" :key="c" :value="c">{{ c }}</option></select></li>
                    <li><span>Año Egreso</span><strong v-if="!editingCards.academic">{{ profileData.anio_egreso || 'N/A' }}</strong><input v-else v-model="profileData.anio_egreso" type="number" :min="ANIO_EGRESO_MIN" :max="ANIO_EGRESO_MAX" class="edit-input" placeholder="1980-2080"/></li>
                </ul>
        ...
        <div class="md:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bento-card relative">
                ...
                <div class="card-content">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all border-2 border-white/10 hover:border-[#a6249d]/40 group">
    ...
    <div v-if="notification.show"
      ...
      <div class="flex-shrink-0">
        <svg class="h-6 w-6 text-[#ff6ec7]" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
