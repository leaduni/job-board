...
<template>
  <div class="min-h-screen bg-gray-900 text-white p-4 sm:p-6 md:p-8">
    <div class="max-w-4xl mx-auto">
      ...
      <!-- Profile Card -->
      <div
        class="bg-gray-800 bg-opacity-40 backdrop-blur-md rounded-2xl shadow-lg p-6 md:p-8"
      >
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <!-- Section: Datos Personales y Académicos -->
          <div class="md:col-span-2">
            <h2
              class="text-xl font-semibold text-gray-300 mb-6 border-b border-gray-700 pb-2"
            >
              Datos Académicos
            </h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-gray-400 text-sm font-bold mb-2"
                  >Nombres</label
                >
                <p v-if="!isEditing" class="text-lg">
                ...
                <input
                  ...
                  type="text"
                  class="w-full bg-gray-900 bg-opacity-50 border border-gray-700 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-fuchsia-500"
                />
              </div>
              ...
              <div>
                <label class="block text-gray-400 text-sm font-bold mb-2"
                  >Ciclo / Año de Egreso</label
                >
                <p v-if="!isEditing" class="text-lg">{{ profileData.cycle }}</p>
                <input
                  v-else
                  v-model="profileData.cycle"
                  type="text"
                  placeholder="Ej. 8vo Ciclo o 2023"
                  class="w-full bg-gray-900 bg-opacity-50 border border-gray-700 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-fuchsia-500"
                />
              </div>
            </div>
          ...
          <div class="md:col-span-1">
            ...
            <div>
              ...
              <a
                ...
                target="_blank"
                class="text-fuchsia-400 hover:text-fuchsia-300 break-all"
                >{{ profileData.linkedin }}</a
              >
              <input
                v-else
          ...
          <div class="md:col-span-3 mt-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <label class="block text-gray-400 text-sm font-bold mb-2"
                  >Skills Técnicos</label
                >
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="skill in profileData.skills"
                    :key="skill"
                    class="flex items-center bg-gray-700 text-gray-200 text-sm font-medium px-3 py-1 rounded-full"
                  >
                    {{ skill }}
                    <button
                      v-if="isEditing"
                      @click="removeSkill(skill)"
                      class="ml-2 text-red-400 hover:text-red-200 text-xs"
                    >
                      &times;
                    </button>
                  </span>
              ...
              <div>
                ...
                <div v-if="!isEditing" class="flex items-center">
                  <svg
                    ...
                    class="h-6 w-6 text-fuchsia-400 mr-2"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <span class="text-gray-300">{{
                    profileData.cvFileName || "No se ha subido CV"
                  }}</span>
                </div>
                <div v-else>
                  <label
                    for="cv-upload"
                    class="cursor-pointer bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg inline-block"
                  >
